...
def cli():
    ...
    parser.add_argument('-s', '--specify-board',  # 备份指定的board 
                        dest='specify_board',
                        nargs = '?',
                        default=False,
                        type= str,
                        help='Backup specific board.' +
                        'If -m or -o args specified, do not specify -s args')
    ...
    print('Backup specific board:', args.specify_board)
    ...
    if args.specify_board: 
        board_name = args.specify_board
        
        list = org_boards_data['me']
        num = len(list)
        for a in range(num, 0, -1):
            a -= 1
            if list[a]['name'] != board_name:
                del list[a]  

    for org, boards0 in org_boards_data.items():
        mkdir(org)
        os.chdir(org)
        boards = filter_boards(boards0, args.closed_boards)
        for board in boards:
